<form action="{{ route('update_book', [ $book->id ]) }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="form-group">
        ...
        <input value="{{ $book->title }}" type="text" name="title" class="form-control" id="title" placeholder="Title ..." />
        <br>
        <label for="descriptions">Description</label>
        <textarea name="descriptions" class="form-control" id="descriptions" rows="20" placeholder="End each line with: \">{{ $book->descriptions }}</textarea>
        <br>
        <label for="url">Book Url Address</label>
        <input value="{{ $book->url }}" type="text" name="url" id="url" class="form-control" placeholder="http://...">
        <br>
