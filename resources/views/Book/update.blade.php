<form action="{{ route('update_book', ['book_id', $book->id ]) }}" method="POST" enctype="multipart/form-data">
    @method('PUT')
    @csrf
    ...
    <div class="form-group">
        ...
        <textarea name="descriptions" class="form-control" id="descriptions" rows="20" placeholder="End each line with: \" required>
            {{ $book->descriptions }}
        </textarea>
        <br>
        ...
        <input type="file" class="form-control" id="book_picture" name="book_picture" required />
    </div>
